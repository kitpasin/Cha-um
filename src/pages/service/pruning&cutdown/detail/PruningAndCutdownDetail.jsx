...
import { ListsData } from "../../../../data/service/pruning&cutdown/ListsData";
import HighlightOffRoundedIcon from "@mui/icons-material/HighlightOffRounded";
import axios from "axios";
import Aos from "aos";
import "aos/dist/aos.css";

export default function PruningAndCutdownDetail() {
  const [pruningAndCutdownDetail, setPruningAndCutdownDetail] = useState([]);
  ...
  const pruningAndCutdownId = location.pathname.split("/")[3];

  // ใช้ method filter เพื่อกรอง ListsData ที่มี id = pruningAndCutdownId
  const filteredPruningAndCutdownDetail = ListsData.filter(
    (list) => list.id.toString() === pruningAndCutdownId
  );
  console.log(filteredPruningAndCutdownDetail);

  // ตัวอย่างการใช้ axios รับ pruning&cutdown detail จาก api
  // async function getPruningAndCutdownDetail() {
  //   const response = await axios.get(`http://localhost:8000/api/service/pruning&cutdown/${pruningAndCutdownId}`);
  //   const data = response.data
  //   setPruningAndCutdownDetail(data)
  // }

  // useEffect(() => {
  //   getPruningAndCutdownDetail();
  // }, [])
  ...
  return (
    <div className="relative md:pt-12 md:pb-24 lg:pb-36 xl:pb-48 2xl:pb-60">
      <figure className="absolute top-0 right-0 max-md:hidden -z-10">
        <img
          src="/images/service/pruning&cutdown/detail/dec-tr.png"
          alt=""
          width={"auto"}
          height={"auto"}
        />
      ...
      <figure className="absolute bottom-0 left-0 max-md:hidden -z-10">
        <img
          src="/images/service/pruning&cutdown/detail/dec-b.png"
          alt=""
          width={"auto"}
          height={"auto"}
        />
      ...
      <div className="relative px-4">
        <div className="bg-[#618861] bg-[url('/images/service/pruning&cutdown/detail/dec-bg.png')] bg-cover bg-center w-full xl:w-[49.6%] h-full absolute top-0 left-0 -z-10" />
          ...
          >
            <div className="flex flex-col gap-4">
              <h1 className="text-4xl font-[500] w-full">
                {filteredPruningAndCutdownDetail[0].title}
              </h1>
              <figure
              ...
              >
                <img
                  ...
                  src="/images/service/pruning&cutdown/detail/detail-1.png"
                  alt={filteredPruningAndCutdownDetail[0].title}
                  width={"auto"}
                  height={"auto"}
                />
              </figure>
            ...
            <div className="flex flex-col gap-12 max-md:gap-4 max-xl:mt-14 max-md:mt-0 flex-none">
              <div className="flex justify-between xs:grid grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 items-center gap-4 text-[18px] xl:text-[22px]">
                ...
                <div className="col-span-3 xl:col-span-4 font-[300]">
                  <p>{filteredPruningAndCutdownDetail[0].address}</p>
                  <p>{filteredPruningAndCutdownDetail[0].size}</p>
                  <p>{filteredPruningAndCutdownDetail[0].status}</p>
                  <p>{filteredPruningAndCutdownDetail[0].type}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="px-4 pb-12 max-md:pt-12">
        ...
        >
          <p className="xl:text-[18px]">{filteredPruningAndCutdownDetail[0].detail}</p>
        </div>
      </div>
      <div className="px-4 pb-12">
        ...
        >
          {filteredPruningAndCutdownDetail[0].subimages.map((subimage) => (
            <div key={subimage.id}>
