...
export default function ParkDetail() {
  ...
  const parkId = location.pathname.split("/")[3];

  // ใช้ method filter เพื่อกรอง ListsData ที่มี id = parkId
  const filteredParkDetail = ListsData.filter(
    (list) => list.id.toString() === parkId
  );
  console.log(filteredParkDetail);

  // ตัวอย่างการใช้ axios รับ park detail จาก api
  // async function getParkDetail() {
  //   const response = await axios.get(`http://localhost:8000/api/service/park/${parkId}`);
  ...
  return (
    <div className="relative md:pt-12 md:pb-24 lg:pb-36 xl:pb-48 2xl:pb-60">
            ...
            <div className="flex flex-col gap-4">
              ...
              >
                <img
                  ...
                  src="/images/service/park/detail/detail-1.png"
                  alt={filteredParkDetail[0].title}
                  width={"auto"}
                  height={"auto"}
                />
              </figure>
            ...
            <div className="flex flex-col gap-12 max-md:gap-4 max-xl:mt-14 max-md:mt-0 flex-none">
              <div className="flex justify-between xs:grid grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 items-center gap-4 text-[18px] xl:text-[22px]">
                ...
                <div className="col-span-3 xl:col-span-4 font-[300]">
                  <p>{filteredParkDetail[0].address}</p>
                  <p>{filteredParkDetail[0].size}</p>
                  <p>{filteredParkDetail[0].status}</p>
                  <p>{filteredParkDetail[0].type}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="px-4 pb-12 max-md:pt-12">
        ...
        >
          <p className="xl:text-[18px]">{filteredParkDetail[0].detail}</p>
        </div>
      </div>
      <div className="px-4 pb-12">
        ...
        >
          {filteredParkDetail[0].subimages.map((subimage) => (
            <div key={subimage.id}>
