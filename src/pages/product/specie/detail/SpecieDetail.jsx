...
import { ListsData } from "../../../../data/product/specie/ListsData";
...
export default function SpecieDetail() {
  ...
  const specieId = location.pathname.split("/")[3];

  // ใช้ method filter เพื่อกรอง ListsData ที่มี id = specieId
  const filteredSpecieDetail = ListsData.lists.filter(
    (list) => list.id.toString() === specieId
  );
  console.log(filteredSpecieDetail);

  // ตัวอย่างการใช้ axios รับ specie detail จาก api
  // async function getSpecieDetail() {
  //   const response = await axios.get(`http://localhost:8000/api/product/specie/${specieId}`);
  //   const data = response.data
  //   setSpecieDetail(data)
  // }
  ...
  return (
    <div className="relative md:pt-12 md:pb-24 lg:pb-36 xl:pb-48 2xl:pb-60">
      <figure className="absolute top-0 right-0 max-md:hidden -z-10">
        <img
          src="/images/product/specie/detail/dec-tr.png"
          alt=""
          width={"auto"}
          height={"auto"}
        />
      ...
      <figure className="absolute bottom-0 left-0 max-md:hidden -z-10">
        <img
          src="/images/product/specie/detail/dec-b.png"
          alt=""
          width={"auto"}
          height={"auto"}
        />
      ...
      <div className="relative px-4">
        <div className="bg-[#618861] bg-[url('/images/product/specie/detail/dec-bg.png')] bg-cover bg-center w-full xl:w-[49.6%] h-full absolute top-0 left-0 -z-10" />
            ...
            <div className="flex flex-col gap-4">
              ...
              >
                <img
                  ...
                  src="/images/product/specie/detail/detail-1.png"
                  alt={filteredSpecieDetail[0].title}
                  width={"auto"}
                  height={"auto"}
                />
              </figure>
            ...
            <div className="flex flex-col gap-12 max-md:gap-4 max-xl:mt-14 max-md:mt-0 flex-none">
              <div className="grid grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 items-center gap-4 text-[18px] xl:text-[22px]">
                ...
                <div className="col-span-3 font-[300]">
                  <p>{filteredSpecieDetail[0].price}</p>
                  <p>{filteredSpecieDetail[0].type}</p>
                  <p>{filteredSpecieDetail[0].tel}</p>
                  <p>{filteredSpecieDetail[0].line}</p>
                </div>
              </div>
