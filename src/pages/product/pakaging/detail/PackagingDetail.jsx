...
export default function PackagingDetail() {
  ...
  const packagingId = location.pathname.split("/")[3];

  // ใช้ method filter เพื่อกรอง ListsData ที่มี id = packagingId
  const filteredPackagingDetail = ListsData.lists.filter(
    (list) => list.id.toString() === packagingId
  );
  console.log(filteredPackagingDetail);

  // ตัวอย่างการใช้ axios รับ packaging detail จาก api
  // async function getPackagingDetail() {
  //   const response = await axios.get(`http://localhost:8000/api/product/packaging/${packagingId}`);
    ...
    <div className="relative md:pt-12 md:pb-24 lg:pb-36 xl:pb-48 2xl:pb-60">
          ...
          >
            <div className="flex flex-col gap-4">
              <h1 className="text-4xl font-[500]">
                {filteredPackagingDetail[0].title}
              </h1>
              <figure
                onClick={handleOpen}
              ...
              >
                <img
                  ...
                  src="/images/product/packaging/detail/detail-1.png"
                  alt={filteredPackagingDetail[0].title}
                  width={"auto"}
                  height={"auto"}
                />
              </figure>
            ...
            <div className="flex flex-col gap-12 max-md:gap-4 max-xl:mt-14 max-md:mt-0 flex-none">
              <div className="grid grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 items-center gap-4 text-[18px] xl:text-[22px]">
                ...
                <div className="col-span-3 font-[300]">
                  <p>{filteredPackagingDetail[0].price}</p>
                  <p>{filteredPackagingDetail[0].type}</p>
                  <p>{filteredPackagingDetail[0].tel}</p>
                  <p>{filteredPackagingDetail[0].line}</p>
                </div>
              </div>
      ...
      <div className="px-4 pb-12 max-md:pt-12">
        <div className="max-w-[1280px] m-auto xl:text-[18px]">
          {filteredPackagingDetail[0].detail}
        </div>
      </div>
    </div>
  );
