...
export default function DesignDetail({ host }) {
  ...
  async function getServiceDetail(serviceId) {
    const response = await axios.get(
      `${host}api/backoffice/v1/design/detail/read/${serviceId}`
    );
  ...
  return (
    <>
      ...
      <Helmet>
        <title>{serviceDetail[0]?.title || "Design"}</title>
        <meta
          name="description"
          content="เรามุ่งมั่นสร้างสรรค์ผลงานที่เป็นเลิศ"
          data-rh="true"
        />
        <link rel="canonical" href={`/etc/design/${serviceDetail[0]?.id}`} />
      </Helmet>
      {!loading ? (
