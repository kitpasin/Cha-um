...
import { ListsData } from "../../../../data/etc/design/ListsData";
...
export default function DesignDetail() {
  ...
  const designId = location.pathname.split("/")[3];

  // ใช้ method filter เพื่อกรอง ListsData ที่มี id = designId
  const filteredDesignDetail = ListsData.filter(
    (list) => list.id.toString() === designId
  );
  console.log(filteredDesignDetail);

  // ตัวอย่างการใช้ axios รับ design detail จาก api
  // async function getDesignDetail() {
  //   const response = await axios.get(`http://localhost:8000/api/etc/design/${designId}`);
  ...
  return (
    <div className="relative md:pt-12 md:pb-24 lg:pb-36 xl:pb-48 2xl:pb-60">
      <figure className="absolute top-0 right-0 max-md:hidden -z-10">
        <img
          src="/images/etc/design/detail/dec-tr.png"
          alt=""
          width={"auto"}
          height={"auto"}
        />
      ...
      <figure className="absolute bottom-0 left-0 max-md:hidden -z-10">
        <img
          src="/images/etc/design/detail/dec-b.png"
          alt=""
          width={"auto"}
          height={"auto"}
        />
      ...
      <div className="relative px-4">
        <div className="bg-[#618861] bg-[url('/images/etc/design/detail/dec-bg.png')] bg-cover bg-center w-full xl:w-[49.6%] h-full absolute top-0 left-0 -z-10" />
            ...
            <div className="flex flex-col gap-4">
              ...
              >
                <img
                  ...
                  src="/images/etc/design/detail/detail-1.png"
                  alt={filteredDesignDetail[0].title}
                  width={"auto"}
                  height={"auto"}
                />
              </figure>
            ...
            <div className="flex flex-col gap-12 max-md:gap-4 max-xl:mt-14 max-md:mt-0 flex-none">
              <div className="flex justify-between xs:grid grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 items-center gap-4 text-[18px] xl:text-[22px]">
                ...
                <div className="col-span-3 xl:col-span-4 font-[300]">
                  <p>{filteredDesignDetail[0].address}</p>
                  <p>{filteredDesignDetail[0].size}</p>
                  <p>{filteredDesignDetail[0].status}</p>
                  <p>{filteredDesignDetail[0].type}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="px-4 pb-12 max-md:pt-12">
        ...
        >
          <p className="xl:text-[18px]">{filteredDesignDetail[0].detail}</p>
        </div>
      </div>
      <div className="px-4 pb-12">
        ...
        >
          {filteredDesignDetail[0].subimages.map((subimage) => (
            <div key={subimage.id}>
